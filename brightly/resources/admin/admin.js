...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".brightly-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.brightly_admin) {
    return;
  }

  var _window$brightly_admi = window.brightly_admin,
      builderStatusData = _window$brightly_admi.builderStatusData,
      getStartedData = _window$brightly_admi.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.brightly-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".brightly-admin-big-notice").addClass("processing");
      $(".brightly-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".brightly-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.brightly-admin-big-notice");
    });
  }
  ...
  $notice_root = $notice_container.closest(".brightly-admin-big-notice");
  $custom_close_button = $notice_root.find(".brightly-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".brightly-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.brightly_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
