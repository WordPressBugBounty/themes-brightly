...
function brightly_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'brightly_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function brightly_theme() {
	return Theme::getInstance();
}

function brightly_assets() {
	return brightly_theme()->getAssetsManager();
}
...
add_action('after_setup_theme', function() {
    brightly_theme()->register_menus(
        array(
            'header-menu' => esc_html__( 'Header Menu', 'brightly' ),
            'footer-menu' => esc_html__( 'Footer Menu', 'brightly' ),
        )
    );
}, 1);
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	brightly_assets()
		...
		->registerStylesheet( 'brightly-theme', '/theme/theme.css' );
	brightly_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		brightly_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'brightly_print_color_scheme', 0 );
} else {
	brightly_assets()
		->registerStyle( 'brightly-theme', brightly_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function brightly_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'brightly_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'brightly_after_activation_redirect_url' );
function brightly_after_activation_redirect_url( $url ) {
...
function brightly_get_builder_plugin_slug() {
...
function brightly_theme_plugins( $plugins ) {

	$plugin_slug = brightly_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'brightly' ) : __( 'Kubio', 'brightly' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'brightly_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === brightly_get_builder_plugin_slug() ) {
                ...
                <div class="kubio-admin-row get-started-imported notice notice-success">
                    <div>
                        <p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'brightly' ),
									brightly_theme()->getName()
								)
							);
							?>
                        </p>
                        <p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'brightly' ),
									brightly_theme()->getName()
								)
							);
							?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'brightly' ) ); ?>
                        </a>
...
function brightly_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'brightly_try_online_url', 10 );

function brightly_render_header_style() {

	?>
    <base target="_top">
	<?php

	brightly_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'brightly_render_header_style', 100 );
