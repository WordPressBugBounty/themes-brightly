...
wp_localize_script(
	...
	array(
		...
		'builderStatusData' => array(
			'status'                          => brightly_theme()->getPluginsManager()->getPluginState( brightly_get_builder_plugin_slug() ),
			'install_url'                     => brightly_theme()->getPluginsManager()->getInstallLink( brightly_get_builder_plugin_slug() ),
			'activate_url'                    => brightly_theme()->getPluginsManager()->getActivationLink( brightly_get_builder_plugin_slug() ),
			'slug'                            => brightly_get_builder_plugin_slug(),
...
<div class="brightly-admin-big-notice--container">
	<div class="logo-holder">
        <h2><?php echo sprintf(__("Choose one of the %s predefined designs to start with:", 'brightly'), ucfirst(get_stylesheet())); ?></h2>
	</div>
	<div class="content-holder">
		<ul class="predefined-front-pages">
            ...
            $front_page_designs =  array(
                array(
                    'name'    => __( 'Modern', 'brightly' ),
                    'index'   => 1,
                    'preview' => 'front-page-1.jpg',
                ),

                array(
                    'name'    => __( 'Fullscreen', 'brightly' ),
                    'index'   => 2,
                    'preview' => 'front-page-2.jpg',
                ),

                array(
                    'name'    => __( 'Classic', 'brightly' ),
                    'index'   => 3,
            ...
            foreach ( $front_page_designs as $brightly_design_index => $design ) : ?>
				<?php
					$brightly_design_selected = $brightly_design_index === 0 ? 'selected' : '';
				...
				<li data-index="<?php echo esc_attr( $design['index'] ); ?>"
					class="<?php echo $brightly_design_selected; ?>">
                    ...
                    <div class="predefined-front-page-card">
						...
						<div class="front-page-design-wrapper">
							<div class="design-preview-image"
								 style="background-image: url(<?php echo esc_attr( brightly_theme()->getAssetsManager()->getBaseURL() . "/images/{$preview_image_name}" ); ?>)"
							></div>
						</div>
